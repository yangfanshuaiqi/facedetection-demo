...
public class Camera2Control implements ICameraControl {
    ...
    @Override
    public void start() {
        // 由于人脸sdk性能影响，选择合适的分辨率，例如720p，横竖屏时，检测想要的宽高是否正确
        if (displayOrientation == CameraView.ORIENTATION_PORTRAIT) {
            if (preferredWidth > preferredHeight) {
                int tmp = preferredWidth;
                preferredWidth = preferredHeight;
                preferredHeight = tmp;
            }
        } else if (displayOrientation == CameraView.ORIENTATION_HORIZONTAL) {
            if (preferredWidth < preferredHeight) {
                int tmp = preferredWidth;
                preferredWidth = preferredHeight;
                preferredHeight = tmp;
    ...
    public void setDisplayOrientation(@CameraView.Orientation int displayOrientation) {
        ...
        // 为了简单起见，只支持应用设置横竖屏，不跟随屏幕旋转而旋转。设置竖屏时，认为屏幕显示选择0，横屏选择90
        if (displayOrientation == CameraView.ORIENTATION_PORTRAIT) {
    ...
    private final TextureView.SurfaceTextureListener surfaceTextureListener =
            new TextureView.SurfaceTextureListener() {
                ...
                public void onSurfaceTextureSizeChanged(SurfaceTexture texture, int width, int height) {
                    Log.i("wtf", "width->" + width + " height->" + height + " " + textureView.getHeight());
    ...
    private final CameraDevice.StateCallback deviceStateCallback = new CameraDevice.StateCallback() {
        ...
        public void onOpened(@NonNull CameraDevice cameraDevice) {
            ...
            Log.i("wtf", "StateCallback onOpened");
        }
        ...
        public void onDisconnected(@NonNull CameraDevice cameraDevice) {
            ...
            Log.i("wtf", "StateCallback onDisconnected");
        }
        ...
        public void onError(@NonNull CameraDevice cameraDevice, int error) {
            ...
            cameraDevice.close();
            Camera2Control.this.cameraDevice = null;
            Log.i("wtf", "StateCallback onError-》" + error);
            start();
        }
    };
...
//                });
            ...
            if (displayRotation == Surface.ROTATION_90) {
                ...
                surfaceTexture.setDefaultBufferSize(previewSize.getWidth(), previewSize.getHeight());
                previewView.setPreviewSize(previewSize.getHeight(), previewSize.getWidth());
            }
            ...
            configureTransform(screenSize.x, screenSize.y);
                ...
                public void onImageAvailable(ImageReader reader) {
                    ...
                    if (cameraFacing == ICameraControl.CAMERA_FACING_FRONT) {
                        if (displayRotation == Surface.ROTATION_0) {
                            rotation = getOrientation(displayRotation);
                        }
...
//                        }
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        Log.i("wtf", "viewWidth:" + viewWidth + " viewHeight:" + viewHeight);
